...
import (
	...
	"fmt"
	"gofound-grpc/global"
	"gofound-grpc/internal/searcher/arrays"
	"gofound-grpc/internal/searcher/model"
	"gofound-grpc/internal/searcher/pagination"
	"gofound-grpc/internal/searcher/sorts"
	"gofound-grpc/internal/searcher/storage"
	"gofound-grpc/internal/searcher/utils"
...
// NewEngine 创建一个引擎
func (e *Engine) NewEngine(name string, c *Container) *Engine {
	e.IndexPath = fmt.Sprintf("%s%c%s", c.Dir, os.PathSeparator, name)
	e.DatabaseName = name
	e.Tokenizer = c.Tokenizer
	e.Shard = c.Shard
	e.Timeout = global.CONFIG.Databases.Timeout
	e.IsDebug = global.CONFIG.Databases.Debug
...
// GetDataBase 获取或创建引擎
func (e *Engine) GetDataBase(name string, c *Container) *Engine {
	//默认数据库名为default
	if name == "" {
		name = "default"
	}
...
func (e *Engine) Init() {
	...
	//初始化文件存储
	for shard := 0; int32(shard) < e.Shard; shard++ {
			...
			if request.ScoreExp != "" {
				...
				for i, doc := range result.Documents {
					...
					if err != nil {
						...
						result.Documents[i].Score = int32(val.(float64))
					}
				}
